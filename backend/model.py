...
def analyze(title,l):
    ...
    for i in range(len(predicted)):
        if(predicted[i][1] > 0.6):
            output[str(i)] = {'review':data[i],'feedback':'Good', 'up':predicted[i][1],'url':metadata[i]['url']}
            num_good +=1
            count+=1
        elif(predicted[i][1]<0.3):
            output[str(i)] = {'review':data[i],'feedback':'Bad','up':(0.95-predicted[i][1]),'url':metadata[i]['url']}
            count +=1 
